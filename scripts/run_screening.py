...
from causal_multiomics_review.llm import CodexCliProvider, OpenAICompatibleProvider
...
def main() -> None:
    ...
    if args.reasoning_effort and args.reasoning_effort != runtime["reasoning_effort"]:
        raise SystemExit(
            "Active suite fixes reasoning effort at "
            f"{runtime['reasoning_effort']}; override is not permitted"
        )
    if provider_config["protocol"] == "codex_cli":
        if args.base_url or args.api_key_env or args.text_verbosity:
            raise SystemExit(
                "--base-url, --api-key-env, and --text-verbosity are not used by Codex CLI"
            )
        provider = CodexCliProvider(
            configured_model,
            codex_bin=args.codex_bin or provider_config.get("codex_bin", "codex"),
            timeout=args.codex_timeout or runtime.get("codex_timeout_seconds", 900),
